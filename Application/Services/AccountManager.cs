    ...
    public class AccountManager : IAccountManager
    {
            ...
            CreateAccountRequest request)
        {
            ...
            if (!PasswordFormat(request.Password))
                throw new ArgumentException("Password must be at least 8 characters and contain " +
        ...
        public async Task<Account?> GetEntityByEmailAsync(string email)
        {
            if (string.IsNullOrWhiteSpace(email))
                throw new ArgumentException("Email is required.");
        ...
        public async Task<AccountResponse?> UpdateAsync(int id, UpdateAccountRequest request)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.Email))
            {
                ...
                if (request.Email != account.Email && await _accountRepository.EmailExistsAsync(request.Email))
                    throw new InvalidOperationException("An account with this email already exists.");
        ...
        public async Task<bool> ReactivateAsync(string email)
        {
            if (string.IsNullOrWhiteSpace(email))
                throw new ArgumentException("Email is required.");
            ...
            if (!account.DeletedAt.HasValue)
                throw new InvalidOperationException("Account deletion date is missing.");
        ...
        public async Task ResetPasswordAsync(Account account, string newPassword)
        {
        ...
        public async Task SetPasswordResetCodeAsync(Account account, string code, DateTime expiresAt)
        {
