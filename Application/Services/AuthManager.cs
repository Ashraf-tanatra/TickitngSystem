...
namespace ApplicationServices.Services
{
    public class AuthManager : IAuthManager
    {
        ...
        public AuthManager(IAccountManager accountManager, IEmployeeManager employeeManager, IEmailService emailService)
        {
        ...
        public async Task<AccountResponse> SignUp(SignUpRequest request)
        {
            ...
            if (request.Password != request.ConfirmPassword)
                throw new ArgumentException(ErrorShared.Account.PasswordsDoNotMatch);

            if (!request.AcceptTerms)
                throw new ArgumentException(ErrorShared.Account.TermsNotAccepted);

            // FORMAT VALIDATION
            if (!_accountManager.ValidEmailFormat(request.Email))
                throw new ArgumentException(ErrorShared.Account.InvalidEmail);

            if (!_accountManager.PasswordFormat(request.Password))
                throw new ArgumentException(ErrorShared.Account.InvalidPassword);

            if (!_employeeManager.ValidPhoneNumberFormat(request.Phone))
                throw new ArgumentException(ErrorShared.Employee.InvalidPhoneNumber);

            // CHECK DUPLICATES=

            if (await _accountManager.ExistsAsync(request.Email))
                throw new InvalidOperationException(ErrorShared.Account.EmailAlreadyExists);

            if (await _employeeManager.ExistsByPhoneAsync(request.Phone))
                throw new InvalidOperationException(ErrorShared.Employee.PhoneAlreadyExists);
            ...
            var verificationCodeExpiresAt = DateTime.Now.AddMinutes(10);
        ...
        public async Task<LoginResponse> Login(LoginRequest request)
        {
            ...
            // GET ACCOUNT           
            var account = await _accountManager.GetEntityByEmailAsync(request.Email);

            // ACCOUNT DOESN'T EXIST            
            if (account == null)
                throw new UnauthorizedAccessException(ErrorShared.Account.InvalidCredentials);
            ...
            // CHECK ACCOUNT STATUS         
            if (account.IsDeleted)
                throw new UnauthorizedAccessException(ErrorShared.Account.AccountDeactivated);

            // CHECK EMPLOYEE STATUS
            if (account.Employee == null || account.Employee.IsDeleted)
                throw new UnauthorizedAccessException(ErrorShared.Account.AccountDeactivated);

            // CHECK EMAIL VERIFICATION           
            if (!account.IsEmailVerified)
                throw new UnauthorizedAccessException(ErrorShared.Account.EmailNotVerified);
        ...
        public async Task ForgotPassword(ForgotPasswordRequest request)
        {
            ...
            var resetCodeExpiresAt = DateTime.Now.AddMinutes(10);

            // SAVE RESET CODE
            await _accountManager.SetPasswordResetCodeAsync(account, resetCode, resetCodeExpiresAt);

            // SEND RESET CODE            
            await _emailService.SendVerificationCodeAsync(account.Email!, resetCode);
        }
        ...
        public async Task ResetPassword(ResetPasswordRequest request)
        {
            ...
            if (string.IsNullOrWhiteSpace(account.PasswordResetCode))
                throw new InvalidOperationException(ErrorShared.Account.InvalidResetCode);
            ...
            if (!account.PasswordResetCodeExpiresAt.HasValue)
                throw new InvalidOperationException(ErrorShared.Account.ResetCodeExpired);
            ...
            // RESET PASSWORD           
            await _accountManager.ResetPasswordAsync(account, request.NewPassword);
        }
